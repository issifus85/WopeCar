...
- **Chat attachments + read-receipt timestamps** (2026-08-20, migration `0066_chat_attachments.sql`): the real server-backed threads (customer Inbox, Ride Support — both booking-anchored via `supabase/migrations/0010_messaging.sql`'s RPCs) support image/camera/document/live-location attachments and a per-message "Read · 3:45 PM" receipt, not just "Read"/"Delivered". `components/MessageThread.js` gained an `onAttach(kind)` prop (`'library'|'camera'|'document'|'location'`) alongside `onSend`; `services/chatAttachmentsApi.js` handles picking/uploading (private `documents` bucket, `documents/chat/<conversationId>/...`, new participant-keyed storage RLS) and opening documents (download-then-`expo-sharing`, same pattern `services/quickbooksApi.js` uses for invoice PDFs). `mark_conversation_read` now fires on every 5s poll tick the thread stays open, not just on mount — the earlier "once on mount only" behavior meant a message sent while the recipient's thread stayed open never actually got marked read. **Rebuild caveat**: document sharing needs `expo-document-picker` (newly added); it ships via OTA fine but won't work on already-installed builds until a full native rebuild — image/camera/location all reuse already-linked modules and work immediately.
- **Vendor Support tab made server-backed** (same day, migration `0067_vendor_support_conversation.sql`): Vendor Mode's Support tab (`app/vendor/(tabs)/support.js`) used to hardcode a fixed local-only `'conv-support'` id — every message a vendor sent there was written to that device's own storage only, never reaching Supabase, so no staff member ever saw it. `conversations.booking_id`/`car_id` can now both be null (a third "general support, no anchor" case, alongside booking- and inquiry-anchored ones — relaxed the old exactly-one XOR constraint to "not both"). New `create_or_get_vendor_support_conversation()` RPC (vendor-gated, mirrors `create_or_get_inquiry_conversation`'s idempotent-singleton pattern) creates a real conversation + participants (vendor + every support/admin user) on first use; `InboxContext.js`'s new `startVendorSupport()` (mirrors `startInquiry()`) resolves it on mount.
...
### 6.12 Vendor payout / marketplace commission

WopeCar takes a platform fee on every booking: the renter pays `bookings.total_cost` in full, WopeCar owes the vendor `bookings.vendor_payout_total` (their own `cars.payout_per_day` × the trip's billable days, admin-set per car — not a percentage of `price_per_day`), and `bookings.wopecar_margin` is the difference. The vendor never sees `total_cost`, `payout_per_day`, or `wopecar_margin` at any point — only their own computed `vendor_payout_total`/`vendor_payout_per_day` for their own bookings.

**Where each field lives:**
- `cars.payout_per_day` — admin-only, set via wopecar-admin's Fleet CarFormModal or the per-vendor payout-rate table (`components/vendors/VendorDetailSlideOver.tsx`, inline-editable). Never appears in any vendor- or renter-facing query in this repo (`services/vendorCarsApi.js`'s `getMyCars`/`createCar`/`updateCarListing` all use an explicit `VENDOR_CAR_SELECT` column list that excludes it, not `select('*')`).
- `bookings.vendor_payout_per_day` / `vendor_payout_total` / `wopecar_margin` / `billable_days` — stamped **server-side** by the `bookings_compute_vendor_payout` `BEFORE INSERT` trigger (Supabase migration `compute_vendor_payout_on_booking_insert`), which looks up the car's `payout_per_day` itself and computes the total from `NEW.billable_days` (a plain day count `checkout/payment.js` sends, computed by the same `calculateRentalPricing()` engine every other cost figure uses — see `constants/pricing.js`'s `calculateVendorPayout`/`calculateWopeCarMargin`). The renter's own device never fetches or handles `payout_per_day` — this was a deliberate change from an earlier draft of this feature that had the client compute the payout, which would have put the admin-only rate into the renter's own JS memory/network traffic.
- **Write protection**: `cars_restrict_vendor_payout_update` trigger blocks any non-admin `UPDATE` that changes `payout_per_day` (narrow deny-list, since `cars_vendor_update` RLS otherwise lets a vendor freely edit their own listing). `restrict_vendor_booking_update`/`restrict_renter_booking_update` (pre-existing triggers, extended) block a vendor/renter from ever writing the 3 booking-level payout/commission columns via a raw `UPDATE`.
- **Read protection**: `vendor_bookings_view` / `vendor_cars_view` (Postgres views, column-projected to exclude the admin-only fields, plus `renter_id` on the bookings view — a pre-existing rule from `getVendorBookings()`'s own doc comment) exist as DB-level defense-in-depth, but the actual app code (`services/vendorBookingsApi.js`, `services/vendorCarsApi.js`) queries the base tables with explicit column lists instead, matching this codebase's existing "explicit list is the guarantee, not RLS" convention (same reasoning as `getVendorBookings()`'s pre-existing `renter_id` exclusion) rather than routing through the new views, since PostgREST's embedded `cars(*)`/`vendors(*)` join support through a fresh view was unverified and this is a security-sensitive, high-traffic path not worth the risk on an unverified pattern.
- `services/vendorBookingsApi.js`'s `normalizeVendorBooking()` maps `vendor_payout_total` to the pre-existing `earnings` field (so `contexts/VendorContext.js`'s aggregates — `currentMonthEarnings`, `deriveEarningsHistory`, `carEarningsThisMonth` — needed no changes) and adds `payoutPending` (true when the car's rate isn't set yet) for the "Payout: Pending"/"Payout: TBC" UI states in `app/vendor/(tabs)/bookings.js` and `app/vendor/history.js`.
- The vendor's real "Booking Confirmed" email (`supabase/functions/send-booking-confirmed`) previously only emailed the renter and an internal admin/support address — never the vendor. Added a third `buildVendorEmailHtml()` send (booking details + a "Your Earnings" section with `vendor_payout_per_day`/`vendor_payout_total` only, no client figures at all).
- Admin-side surfaces: wopecar-admin's `CarFormModal`/`CarDetailSlideOver` (Fleet, admin-only pricing breakdown card), `BookingDetailSlideOver` (navy/teal/orange "Financial Summary" section), the Bookings table (Booking Total/Vendor Payout/Commission columns), the Dashboard (`Gross Bookings`/`Vendor Payouts`/`WopeCar Commission` KPI cards + a 3-bar monthly chart), and `VendorDetailSlideOver`'s per-car table (Client Rate/Payout Rate/Commission per day/Commission %, inline-editable).
...
**Real QuickBooks invoicing** (Change 8, `pending_invoices` table, migration 0065): the flagged architecture conflict was resolved as **Option C — a dedicated table**, not a new `bookings.status` value, deliberately kept isolated from `bookings`'s RLS/triggers/status labels. `handleSaveToCart` inserts a `pending_invoices` row (same column shapes as `bookings`, own `booking_ref`) alongside the local saved-cart draft, then fires `quickbooks-create-pending-invoice` (wopecar-admin Edge Function) fire-and-forget — it builds the invoice via the same `buildInvoiceLineItems()`/`buildInvoiceMemo()` helpers `quickbooks-create-invoice` uses (both live in `_shared/quickbooks.ts`, so the two paths can't drift), sets `DueDate` = `expires_at`, and emails the renter their invoice PDF. `CartContext`'s expiry prune calls `quickbooks-void-invoice` on lapse (voids the QB invoice + sends the expiry email); `handlePay()`'s resume path reads the saved draft's `pendingInvoiceId` and reuses its existing QB invoice (via `getPendingInvoiceQuickbooksRef` + a plain `updateBooking()` write of the 3 `qb_*` ref columns) instead of minting a duplicate, degrading gracefully to a fresh invoice if the fire-and-forget create hasn't landed yet; `quickbooks-record-payment` marks the `pending_invoices` row `resolution: 'paid'` in the same call (RLS gives renters no update policy on that table past the initial insert — every post-insert mutation is service-role only). wopecar-admin has a matching **Pending Payment** tab (`app/(admin)/bookings/pending-payment`, `lib/api/pendingInvoices.ts`) with Send Reminder (`quickbooks-send-payment-reminder`) / Void Booking actions, plus a dashboard **QuickBooks** summary card (`components/dashboard/QuickBooksSummaryCard.tsx`).
...
| Vendor payout | `cars.payout_per_day` (admin-only, GHS/day) via `calculateVendorPayout(payoutPerDay, days)`; commission via `calculateWopeCarMargin(clientTotal, vendorPayout)` | Computed server-side at booking creation, never client-side — see 6.12 |

Source for the original 6 rules: `wopecar.com/faq` (live FAQ page) and `wopecar.com`'s booking widget/T&Cs — cross-checked and confirmed matching before the min-booking-days rule was added. WopeCare's rates/coverage/copy were supplied directly by the user, not scraped from a live source (WopeCare didn't exist on the site before this feature). The vendor payout rule was supplied directly by the user as a new marketplace-commission feature (Section 6.12).
