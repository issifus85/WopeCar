// Supabase Edge Function - notifies a user by email when admin verifies or
// rejects their driver's licence or national/government ID. Caller must be
// an admin (wopecar-admin's lib/api/documents.ts and mobile's
// services/adminUsersApi.js's setUserVerificationStatus() are the real call
// sites) - identified from their own JWT, then role verified via
// service_role before anything is sent. Mirrors send-vendor-document-status
// almost exactly, just looking the target user up directly instead of via
// a vendors join.
//
// Deploy with: supabase functions deploy send-id-verification-status
    ...
    <div style="max-width:480px;margin:0 auto;background:#ffffff;border-radius:16px;overflow:hidden;">
      ...
      <div style="padding:28px 24px;">
        ...
        <div style="text-align:center;margin-top:24px;">
          <a href="wopecar://account" style="display:inline-block;background:#154B59;color:#ffffff;font-weight:bold;font-size:14px;padding:12px 28px;border-radius:10px;text-decoration:none;">Update in the app</a>
        </div>` : ''}
      </div>
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { userId, docLabel, verified, reason } = await req.json();
    if (!userId || typeof verified !== 'boolean' || !docLabel) {
      return jsonResponse({ error: 'userId, docLabel, and verified are required.' }, 400);
    }
    ...
    const [{ data: targetUser }, { data: supportSetting }] = await Promise.all([
      adminClient.from('users').select('email, full_name').eq('id', userId).maybeSingle(),
      adminClient.from('app_settings').select('value').eq('key', 'support_email').maybeSingle(),
    ]);
    if (!targetUser?.email) {
      return jsonResponse({ error: 'User has no email on file.' }, 404);
    }
    ...
    const firstName = (targetUser.full_name || '').trim().split(/\s+/)[0] || 'there';
