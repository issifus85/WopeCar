...
// car_sale_inquiries row is created. Called by the public website's Buy a
// Car "Enquire" modal (CarSaleInquiryForm.tsx) right after its own anon
// insert.
//
// Kept separate from send-inquiry-notification: that function's inquiries
// join to the rental `cars` table, while these join to car_sale_listings -
// a different foreign entity with its own admin surface (Cars For Sale).
// Otherwise this mirrors it exactly, including the security and
// idempotency reasoning below.
//
// Unlike most notification functions in this project, the caller here is
// an anonymous website visitor - there is no Supabase Auth session on the
// public site, so this function can't identify "the caller who owns this
// row" from a JWT. verify_jwt is therefore off for this function. The only
// input trusted from the client is the inquiryId; every field that ends up
...
// nonexistent) inquiry id.
//
// The notified_at column makes this idempotent: a retried or duplicated
// call for the same id is a no-op, which also caps the blast radius of
// someone replaying a known id to spam the support inbox - they get at
// most the one real email that already fired.
//
// Deploy with: supabase functions deploy send-car-sale-inquiry-notification --no-verify-jwt
...
function buildEmailHtml({ inquiry, listing, heading, intro, ctaHref, ctaLabel }: any) {
  const body = `
      <div style="padding:28px 24px;">
        ...
        <div style="background:#f5f5f5;border-radius:12px;padding:16px;margin-bottom:16px;">
          <div style="font-size:16px;font-weight:bold;color:#154B59;">${escapeHtml(listing.name)}</div>
          <div style="font-size:13px;color:#5b6b6c;margin-top:2px;">GH₵${Number(listing.price ?? 0).toLocaleString()}${listing.location ? ` · ${escapeHtml(listing.location)}` : ''}</div>
        </div>` : ''}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: inquiry, error: inquiryError } = await adminClient
      .from('car_sale_inquiries')
      .select('*, listing:car_sale_listings(name, price, location)')
      .eq('id', inquiryId)
      .maybeSingle();
    ...
    if (supportEmail) {
      try {
        await sendResendEmail(resendApiKey, {
          ...
          to: [supportEmail],
          subject: `New Car Sale Enquiry - ${inquiry.listing?.name ?? 'Buy a Car'}`,
          html: buildEmailHtml({
            inquiry,
            listing: inquiry.listing,
            heading: 'New Car Sale Enquiry',
            intro: inquiry.listing ? `Submitted from the ${inquiry.listing.name} listing on Buy a Car.` : 'Submitted from the Buy a Car page.',
            ctaHref: 'https://admin.wopecar.com/content/pages/buy-a-car',
            ctaLabel: 'View Buy a Car in Admin',
          }),
        });
      } catch (e) {
    ...
    await adminClient.from('car_sale_inquiries').update({ notified_at: new Date().toISOString() }).eq('id', inquiryId);
