// Supabase Edge Function - notifies a vendor by email when admin approves
// or rejects their vendor application. Caller must be an admin (services/
// adminVendorsApi.js's approveVendor()/rejectVendor() are the only real
// call sites) - identified from their own JWT, then role verified via
// service_role before anything is sent.
//
// Takes vendorUserId/businessName directly in the request body rather than
// re-fetching the vendor row by id - rejectVendor() deletes the vendors row
// before this runs (see its own docstring), so a re-fetch would find
// nothing for the rejection case. The caller already has these fields in
// hand from its own read, so passing them through is simpler than trying
// to beat a deletion race.
//
// Deploy with: supabase functions deploy send-vendor-application-status
...
function buildEmailHtml({ firstName, businessName, approved, reason, supportEmail }: {
  firstName: string; businessName: string; approved: boolean; reason: string | null; supportEmail: string | null;
}) {
  const icon = approved ? '&#127881;' : '&#128203;';
  const iconBg = approved ? '#EEF9F9' : '#FDECEC';
  const title = approved ? 'Application Approved!' : 'Application Not Approved';
  const message = approved
    ? `Great news - ${escapeHtml(businessName)} is now an approved WopeCar vendor. You can start listing cars right away.`
    : `Your application for ${escapeHtml(businessName)} was not approved this time.`;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { vendorUserId, businessName, approved, reason } = await req.json();
    if (!vendorUserId || typeof approved !== 'boolean') {
      return jsonResponse({ error: 'vendorUserId and approved are required.' }, 400);
    }
    ...
    // Step 1: identify the caller and verify they're actually an admin.
    ...
    // Step 2: fetch the vendor's own contact info (never trust the caller
    // to supply it) and send.
    ...
    const resendRes = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        to: [vendorUser.email],
        subject: approved ? 'Your WopeCar vendor application was approved' : 'Update on your WopeCar vendor application',
        html,
      }),
    });
