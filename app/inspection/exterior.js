...
export default function InspectionExteriorScreen() {
  ...
  const [pendingTap, setPendingTap] = useState(null); // {xPct, yPct} while choosing a damage type
  ...
  // react-native-web's Pressable doesn't reliably populate
  // nativeEvent.locationX/locationY the way native RN does (confirmed live -
  // it silently produced NaN, which JSON.stringify then turns into null,
  // which then fails the backend's numeric validation on sync). pageX/pageY
  // are real DOM MouseEvent properties on web and standard RN touch
  // properties on native, so measuring the diagram's on-screen position via
  // measure() and subtracting works correctly cross-platform.
  ...
  const handleContinue = async () => {
    ...
    router.push({ pathname: '/inspection/interior', params: { bookingId, localId, type } });
  };
